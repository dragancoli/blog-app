...
exports.getAllPosts = async (req, res) => {
  try {
    const posts = await db.query(
      "SELECT p.id, p.title, p.created_at, p.image_url, u.username as author FROM posts p JOIN users u ON p.author_id = u.id ORDER BY p.created_at DESC"
    );
    res.json(posts.rows);
  } catch (err) {
    res.status(500).send("Greška na serveru");
...
exports.updatePost = async (req, res) => {
  try {
    ...
    if (post.rows[0].author_id !== req.user.id)
      return res.status(403).json({ msg: "Nemate dozvolu." });
