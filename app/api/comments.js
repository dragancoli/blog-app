...
export const createComment = async (postId, content, parent_id = null) => {
  ...
  const res = await axios.post(`${API_BASE}/posts/${postId}/comments`, { content, parent_id }, { headers });
  return res.data;
};

export const updateComment = async (id, content) => {
  const headers = await authHeader();
  const res = await axios.put(`${API_BASE}/comments/${id}`, { content }, { headers });
  return res.data;
};

export const deleteComment = async (id) => {
  const headers = await authHeader();
  const res = await axios.delete(`${API_BASE}/comments/${id}`, { headers });
  return res.data;
};
